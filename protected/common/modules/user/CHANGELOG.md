...
- Enh: Custom side menu via `common\modules\user\Module::items` (mdmunir).
...
- Chg: Can customize return of `common\modules\user\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `common\modules\user\components\BizRule` (mdmunir).
...
- Chg: Remove `common\modules\user\Module::allowActions`. Set access control directly with `common\modules\user\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `common\modules\user\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
